...
const getEmailFromFeedback = (feedback: Feedback): string | undefined => {
  const emailAnswer = feedback.answers.find(
    (a) =>
      ...
      a.title.toLowerCase().includes("mail")
  );
    ...
    <div className="p-4 sm:p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground">{t.feedback.manageDesc}</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <Select value={featureFilter} onValueChange={setFeatureFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="translate_cv">
                {featureLabels.translate_cv}
              </SelectItem>
              <SelectItem value="generate_cv">
                {featureLabels.generate_cv}
              </SelectItem>
              <SelectItem value="generate_cl">
                {featureLabels.generate_cl}
              </SelectItem>
              <SelectItem value="rebuild_cv_pdf">
                {featureLabels.rebuild_cv_pdf}
              </SelectItem>
              <SelectItem value="ai_interview">
                {featureLabels.ai_interview}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filteredFeedbacks.map((fb) => {
            ...
            const colorClass =
              featureColors[fb.feature] ?? "bg-slate-100 text-slate-700";
            return (
      ...
      >
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{t.feedback.detailDesc}</DialogDescription>
          </DialogHeader>
          {selectedFeedback && (
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <Badge
                  className={
                    featureColors[selectedFeedback.feature] ??
                    "bg-slate-100 text-slate-700"
                  }
                >
                  {featureLabels[selectedFeedback.feature] ??
                    selectedFeedback.feature}
                </Badge>
                <span className="text-muted-foreground">
                  Thời gian:{" "}
                  {formatDateTime(
                    selectedFeedback.submittedAt || selectedFeedback.createdAt
                  )}
                </span>
              </div>
