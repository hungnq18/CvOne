        ...
        const fetchLocationNames = async () => {
            try {
                if (application.company_city) {
                    ...
                    const city = provinces.find(p => p.code === Number(application.company_city))
                    if (city) {
                ...
                if (application.company_city && application.company_district) {
                    const districts = await getDistrictsByProvinceCode(Number(application.company_city))
                    const district = districts.find(d => d.code === Number(application.company_district))
                    if (district) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full h-[90vh] max-h-[90vh] sm:h-auto sm:max-h-[90vh] md:max-w-2xl overflow-y-auto rounded-lg border border-border bg-card shadow-lg">
                ...
                <div className="sticky top-0 flex items-center justify-between border-b border-border bg-card px-4 py-3 sm:px-6 sm:py-4">
                    <h2 className="text-lg sm:text-xl font-bold">{t.admin.newHr.modal.title}</h2>
                    ...
                    <div>
                        ...
                        <div className="grid gap-3 sm:gap-6 grid-cols-1 sm:grid-cols-2">
                            <div>
                                ...
                                <p className="mt-1 text-sm sm:text-lg">{application.first_name}</p>
                            </div>
                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.lastName}</label>
                                <p className="mt-1 text-sm sm:text-lg">{application.last_name}</p>
                            </div>
                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.phone}</label>
                                <p className="mt-1 text-sm sm:text-lg">{application.phone || "—"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid gap-3 sm:gap-6 grid-cols-1 sm:grid-cols-2">
                            <div className="sm:col-span-2">
                                ...
                                <p className="mt-1 text-sm sm:text-lg">{application.company_name}</p>
                            </div>

                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.taxId}</label>
                                <p className="mt-1 text-sm sm:text-lg font-mono">{application.vatRegistrationNumber}</p>
                            </div>

                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.country}</label>
                                <p className="mt-1 text-sm sm:text-lg">{application.company_country}</p>
                            </div>

                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.city}</label>
                                <p className="mt-1 text-sm sm:text-lg">{cityName || application.company_city}</p>
                            </div>

                            <div>
                                <label className="text-xs font-semibold">{t.admin.newHr.modal.district}</label>
                                <p className="mt-1 text-sm sm:text-lg">{districtName || application.company_district}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid gap-3 sm:gap-6 grid-cols-1 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm sm:text-lg">{t.admin.newHr.modal.statusPending}</p>
                            </div>
                        </div>
                    </div>
            ...
            <AlertDialog open={isConfirmRejectOpen} onOpenChange={setIsConfirmRejectOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{t.admin.newHr.modal.confirmRejectTitle}</AlertDialogTitle>
                        <AlertDialogDescription>
                            {t.admin.newHr.modal.confirmRejectDesc}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>{t.admin.newHr.modal.confirmRejectCancel}</AlertDialogCancel>
