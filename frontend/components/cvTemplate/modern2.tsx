...
const Section: React.FC<SectionProps> = ({ title, children, sectionId, onSectionClick, isPdfMode = false }) => {
  return (
    <div className="mb-6">
      <HoverableWrapper label={title} sectionId={sectionId} onClick={onSectionClick} isPdfMode={isPdfMode}>
        <div className="bg-card rounded-xl p-8 shadow-md border-2 border-border elegant-hover relative overflow-hidden">
          ...
          <div className="mb-4 relative">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="font-serif text-xl font-semibold text-foreground tracking-tight">{title}</h2>
            </div>
...
// --- MAIN COMPONENT ---
const Modern2: React.FC<Modern2Props> = ({ data, onSectionClick, isPdfMode = false, language }) => {
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-8 py-6">
              <div className="flex items-center gap-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-blue-300 text-base font-medium tracking-wide uppercase">{professionalTitle}</p>
                </div>
              </div>
            </div>
          </HoverableWrapper>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                {userData.dateOfBirth && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-700 font-medium">{userData.dateOfBirth}</span>
                  </div>
                )}
                {userData.gender && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-700 font-medium">{userData.gender}</span>
                  </div>
                )}
                {userData.phone && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-700 font-medium">{userData.phone}</span>
                  </div>
                )}
                {userData.email && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-700 font-medium break-all">{userData.email}</span>
                  </div>
                )}
            ...
            {userData.summary && (
              ...
              >
                <div className="professional-card bg-white p-6 border-l-4 border-blue-600">
                  <p className="text-slate-700 leading-relaxed text-pretty">{userData.summary}</p>
                </div>
              </Section>
            )}
                ...
                <div className="space-y-4">
                  {userData.workHistory.map((job: any, i: number) => (
                    <div key={i} className="professional-card bg-slate-50 p-4 border-l-4 border-slate-700">
                      ...
                      <div className="mt-3">{renderDescription(job.description)}</div>
                    </div>
                  ))}
                </div>
              </Section>
