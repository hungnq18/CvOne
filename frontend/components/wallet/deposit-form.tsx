...
import {
    ArrowRight, Loader, DollarSign, AlertCircle, Tag, X, Check, QrCode, CreditCard
} from "lucide-react"
...
export default function DepositForm() {
    ...
    const handleApplyVoucher = async () => {
        ...
        try {
            ...
            const foundVoucher = allVouchers.find((v: Voucher) => v.name.toLowerCase() === voucherName)
            ...
            if (!voucherData.isActive) { setVoucherError(t.voucherInactive); setVoucher(null); return }
            if (now < startDate) { setVoucherError(t.voucherNotStarted); setVoucher(null); return }
            if (now > endDate) { setVoucherError(t.voucherExpired); setVoucher(null); return }
            if (voucherData.minOrderValue && amount < voucherData.minOrderValue) {
                setVoucherError(t.voucherMinOrder.replace("{min}", formatVnd(voucherData.minOrderValue)))
                setVoucher(null)
                return
            }
    ...
    const handleProceedToCheckout = async () => {
        if (amount <= 0) { setError(t.amountInvalid); return }
        if (amount < MIN_AMOUNT) { setError(t.amountTooSmall.replace("{min}", formatVnd(MIN_AMOUNT))); return }
        if (amount > MAX_AMOUNT) { setError(t.amountTooLarge.replace("{max}", formatVnd(MAX_AMOUNT))); return }
        if (voucher && voucher.minOrderValue && amount < voucher.minOrderValue) {
            setError(t.voucherMinOrder.replace("{min}", formatVnd(voucher.minOrderValue)))
            return
        }
        ...
        try {
            ...
            if (paymentMethod === "qr") {
                ...
                else { setError("Cannot create payment link"); setIsLoading(false) }
            } else {
        ...
        <div className="space-y-8">
            ...
            {error && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <button onClick={() => setError(null)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200">×</button>
                </div>
            )}

            {/* Deposit Packages */}
            <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t.selectPackage}</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {DEPOSIT_PACKAGES.map(pkg => (
                        <button
                            key={pkg.id}
                            onClick={() => { setSelectedPackage(pkg.id); setUseCustom(false); setError(null) }}
            ...
            {/* Custom Amount */}
            <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t.customAmount}</h2>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                    {t.minAmount}: {formatVnd(MIN_AMOUNT)}, {t.maxAmount}: {formatVnd(MAX_AMOUNT)}
                </p>
            </div>

            {/* Payment Method */}
            <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t.paymentMethod}</h2>
                    ...
                    {["qr", "card"].map(method => {
                        ...
                        return (
                            <button
                                key={method}
                                type="button"
                                onClick={() => !isDisabled && setPaymentMethod(method as "qr" | "card")}
                                disabled={isDisabled}
                        ...
                        ${isDisabled ? "opacity-50 cursor-not-allowed" : ""}`}
                            ...
                            ${paymentMethod === method && !isDisabled
                                    ...
                                    >
                                        {method === "qr" ? <QrCode className="w-6 h-6" /> : <CreditCard className="w-6 h-6" />}
                                    </div>
                                    <div className="text-center">
                                        <p className={`font-semibold ${paymentMethod === method && !isDisabled ? "text-blue-600 dark:text-blue-400" : "text-slate-900 dark:text-white"}`}>
            ...
            {/* Voucher Code */}
            <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t.voucher}</h2>
                {voucher ? (
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-green-600 dark:text-green-400">
                                    Giảm: {voucher.discountType === "percent" ? `${voucher.discountValue}%` : `${formatVnd(voucher.discountValue)}đ`}
                                </p>
                            </div>
                            <button onClick={handleRemoveVoucher} className="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-200 ml-4">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {validatingVoucher ? <><Loader className="w-4 h-4 animate-spin" />{t.checking}</> : t.apply}
                        </button>
                    </div>
                )}
                {voucherError && <p className="text-sm text-red-600 dark:text-red-400 mt-2">{voucherError}</p>}
            </div>

            {/* Order Summary */}
            {amount > 0 && (
                <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t.orderSummary}</h3>
                    <div className="space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-slate-600 dark:text-slate-400">{t.depositAmount}</span>
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-slate-600 dark:text-slate-400">{t.amountAfterDiscount}</span>
                            <span className="font-semibold text-slate-900 dark:text-white">{formatVnd(amountAfterDiscount)}đ</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-slate-600 dark:text-slate-400">{t.transactionFee}</span>
                            <span className="font-semibold text-slate-900 dark:text-white">{formatVnd(Math.ceil(transactionFee))}đ</span>
                        </div>
                        <div className="flex justify-between items-center pt-3">
                            <span className="text-lg font-semibold text-slate-900 dark:text-white">{t.total}</span>
                            <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{formatVnd(Math.ceil(totalAmount))}đ</span>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex gap-3">
                <Link href="/user/wallet" className="flex-1 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-semibold py-3 px-6 rounded-lg transition-colors text-center">{t.cancel}</Link>
                <button
                ...
                >
                    {isLoading ? <><Loader className="w-4 h-4 animate-spin" />{t.processing}</> : <>{t.proceed}<ArrowRight className="w-4 h-4" /></>}
                </button>
            </div>
        </div>
    )
