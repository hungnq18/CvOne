...
import {
    ArrowRight, Loader, AlertCircle, Tag, X, Check, QrCode, CreditCard
} from "lucide-react"
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            const trimmed = numericOnly.replace(/^0+/, "") || ""
                            const amountValue = trimmed ? Number.parseInt(trimmed) : 0

                            let nextValue = trimmed
                            if (amountValue > MAX_AMOUNT) {
                                nextValue = MAX_AMOUNT.toString()
                                setError(t.amountTooLarge.replace("{max}", formatVnd(MAX_AMOUNT)))
                            } else if (amountValue > 0 && amountValue < MIN_AMOUNT) {
                                setError(t.amountTooSmall.replace("{min}", formatVnd(MIN_AMOUNT)))
                            } else {
                                setError(null)
                            }
                            ...
                            if (voucher && voucher.minOrderValue && amountValue < voucher.minOrderValue) {
                                handleRemoveVoucher()
                            }
                        }}
