...
const FastApplyModal = dynamic(
  () => import("@/components/modals/FastApplyModal"),
  {
    ssr: false,
    loading: () => null,
  }
...
const translations = {
  vi: {
    ...
    mustProvideCoverletterIdOrCoverletterUrl:
      "Phải cung cấp ít nhất Cover Letter ID hoặc URL Cover Letter",
    ...
    cannotApplyOwnJob:
      "Bạn không thể ứng tuyển cho công việc do chính mình tạo!",
  },
  en: {
    ...
    mustProvideCoverletterIdOrCoverletterUrl:
      "Must provide at least Cover Letter ID or Cover Letter URL",
  ...
  return (
    <div className="min-h-screen">
      <div className="container mx-auto p-4 sm:p-6 lg:p-8">
        <Link
          href="/jobPage"
          className="text-blue-500 hover:text-blue-700 mb-6 inline-flex items-center gap-2"
        >
          ...
          <div className="lg:col-span-2 bg-white p-6 sm:p-8 rounded-xl shadow-xl">
            ...
            <div className="border-l-4 border-blue-500 p-4 rounded-md bg-white">
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-700 text-sm">
                  <strong className="font-medium">{t.experience}</strong>{" "}
                  {job.experience}
                </p>
                <div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              <div className="flex flex-col w-full p-2 rounded-3xl gap-2">
                <div
                  className={`apply-split-group group ${
                    split ? "apply-split-split" : ""
                  }`}
                  onMouseEnter={() => {
                    setIsApplyHovered(true);
                    splitTimeout.current = window.setTimeout(
                      () => setSplit(true),
                      80
                    );
                  }}
                ...
                <button
                  className={`bg-indigo-600 text-white shadow-lg hover:bg-indigo-700 hover:scale-105 hover:brightness-110 hover:shadow-xl transition-all duration-200 font-semibold px-6 py-2 rounded-2xl text-center cursor-pointer w-full ${
                    ...
                    try {
                      ...
                      if (!skipCoverLetter) {
                        if (
                          ...
                          const msg =
                            t.selectCL + " - " + t.chooseCLFromLibrary;
                          setFormError(msg);
                          message.error(msg);
                          return;
                        }
                        if (clMode === "upload" && !clUploadFile) {
                          const msg =
                            t.selectCL + " - " + t.uploadCLFromComputer;
                          setFormError(msg);
                          message.error(msg);
                          return;
                        }
                      ...
                      if (applyMode === "upload" && cvUploadFile) {
                        ...
                        cvUrl =
                          cv &&
                          (cv as any).content &&
                          (cv as any).content.userData
                            ? (cv as any).content.userData.fileUrl
                            : undefined;
                      ...
                      if (!skipCoverLetter) {
                        if (clMode === "upload" && clUploadFile) {
                          coverletterUrl = await uploadFileToCloudinary(
                            clUploadFile
                          );
                          clId = undefined;
                          ...
                          coverletterUrl =
                            cl &&
                            (cl as any).content &&
                            (cl as any).content.userData
                              ? (cl as any).content.userData.fileUrl
                              : undefined;
                      ...
                      if (errorMsg.includes("Cloudinary")) {
                        ...
                        errorMsg.includes("cvId hoặc cvUrl") ||
                        errorMsg.includes("cvId or cvUrl")
                      ) {
                        message.error(t.mustProvideCvIdOrCvUrl);
                      } else if (
                        errorMsg.includes(
                          "coverletterId hoặc coverletterUrl"
                        ) ||
                        errorMsg.includes("coverletterId or coverletterUrl")
                      ) {
                        message.error(
                          t.mustProvideCoverletterIdOrCoverletterUrl
                        );
                      } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedJobs.map((relatedJob) => (
                <Link href={`/jobPage/${relatedJob._id}`} key={relatedJob._id}>
                  <Card
                    hoverable
                    className="transition-all duration-200 border-blue-200 hover:shadow-xl hover:border-blue-400"
                  >
