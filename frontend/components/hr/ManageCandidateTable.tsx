...
const ManageCandidateTable = () => {
  ...
  const handleDownloadCV = async (cvData: any) => {
    ...
    if (
      !cvData ||
      !cvData.content?.userData ||
      !(cvData.cvTemplateId || cvData.templateId)
    ) {
  ...
  const filteredApplications = applications
    .filter((app) => {
      ...
      const createdAt = new Date(
        app.createdAt || app.updatedAt || app.submit_at || 0
      );
    ...
    .sort((a, b) => {
      const dateA = new Date(
        a.updatedAt || a.createdAt || a.submit_at || 0
      ).getTime();
      const dateB = new Date(
        b.updatedAt || b.createdAt || b.submit_at || 0
      ).getTime();
      return dateB - dateA;
    });
  ...
  const workTypes = Array.from(
    new Set(
      applications
        .map((app) => app.jobId?.workType || app.jobId?.["Work Type"])
        .filter(Boolean)
    )
  );
            ...
            {filteredApplications.length === 0 ? (
              ...
              filteredApplications.map((app: any) => (
                <TableRow key={app._id}>
                  ...
                  <TableCell>{app.userId?.phone || "-"}</TableCell>
                  <TableCell>{app.jobId?.title || app.job_id || "-"}</TableCell>
                  <TableCell>
                    {app.updatedAt
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleViewCoverLetter(
                            app.coverletterId?._id || app.coverletter_id
                          )
                        }
                        style={{
                          background: "#f5f5f5",
                      ...
                      <DownloadButton
                        onClick={() => setDownloadModal({ open: true, app })}
                      />
                      <ChatButton
                        participantId={
                          app.userId?._id ||
                      ...
                      <DeleteButton
                        onClick={() =>
                          setDeleteModal({ open: true, appId: app._id })
                        }
                      />
                    </div>
                  </TableCell>
              ...
              onClick={async () => {
                if (deleteModal.appId) {
                  ...
                  setApplications((apps) =>
                    apps.filter((a) => a._id !== deleteModal.appId)
                  );
                }
