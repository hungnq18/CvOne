...
export default function ManageJobPage() {
      ...
      if (Array.isArray(jobsArr)) {
        setJobs(
          jobsArr.map((job: any) => ({
            ...
            "Job Posting Date":
              job.postingDate || job["Job Posting Date"] || "",
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const matchStatus =
      statusFilter === "all" ||
      (job.isActive ? "Active" : "Inactive") === statusFilter;
    const matchWorkType =
      workTypeFilter === "all" || job["Work Type"] === workTypeFilter;
  ...
  const sortedJobs = [...filteredJobs].sort((a, b) => {
    if (sortOption === "newest") {
      return (
        new Date(b["Job Posting Date"]).getTime() -
        new Date(a["Job Posting Date"]).getTime()
      );
    ...
    if (!newJob.applicationDeadline) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(newJob.applicationDeadline)) {
        newErrors["applicationDeadline"] =
          "Application Deadline must be in format YYYY-MM-DD";
  ...
  const handleEditJob = async () => {
    ...
    if (applicationDeadlineISO) {
      try {
        ...
        toast({
          title: "Error",
          description: "Invalid application deadline format",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 space-y-6 max-w-full">
      ...
      <Card>
        <CardHeader className="space-y-2">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mt-2">
            ...
            <Button
              className="w-full mt-2 sm:ml-auto sm:w-auto sm:mt-0 flex items-center gap-2"
      ...
      <JobEditModel
        ...
        onChange={(job) =>
          isEditDialogOpen ? setSelectedJob(job as Job) : setNewJob(job)
        }
