...
const PageCreateCVAIContent = () => {
  ...
  const handleSectionClick = (sectionId: string, event?: React.MouseEvent) => {
    ...
    if (event && (event.target as HTMLElement).closest('.section-toggle-icon')) {
      ...
      if (isInCV) {
        ...
        const { place, order } = calculatePlaceAndOrder(sectionId, positionsCopy, currentTemplate.title);
  ...
  const handleTranslateCV = async (targetLanguage: string) => {
    ...
    try {
      ...
      const defaultLabels = language === "vi" ? {
      ...
      console.log("[handleTranslateCV] API Response structure:", {
        ...
        uiTextsKeys: translatedData?.data?.uiTexts ? Object.keys(translatedData.data.uiTexts) : [],
        fullPath: {
      ...
      const nextUserData = translatedData?.data?.content?.userData ?? translatedData?.data?.data?.content?.userData;
      const nextUiTexts = translatedData?.data?.uiTexts ?? translatedData?.data?.data?.uiTexts;
  ...
  const handleAISuggestTemplate = async () => {
     ...
     try {
       ...
       notify.error(language === "vi" ? "AI đề xuất mẫu thất bại" : "AI suggestion failed");
     } finally {
