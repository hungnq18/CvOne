...
export function useWebSpeechToText(
  ...
  useEffect(() => {
    ...
    const detectLanguageFromText = (text: string): string | null => {
      ...
      const vietnameseWords = /\b(của|và|với|cho|từ|về|trong|này|đó|được|sẽ|có|không|một|hai|ba|bạn|tôi|chúng|nó|họ)\b/i;
      
      // English common words
      const englishWords = /\b(the|is|are|and|or|but|in|on|at|to|for|of|with|by|this|that|these|those|was|were|been|have|has|had|do|does|did|will|would|should|could|can|may|might)\b/i;
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (result.isFinal) {
          ...
          if (confidence >= minConfidence) {
            ...
            console.warn(`Low confidence result filtered: ${transcript} (confidence: ${confidence})`);
      ...
      if (options.autoSwitchLanguage && recognitionRef.current && isRecordingRef.current) {
        const fullText = (finalTranscriptRef.current + ' ' + finalTranscript + ' ' + interimTranscript).trim();
        if (fullText.length > 10) { // Only switch if we have enough text
            ...
            languageSwitchTimeoutRef.current = setTimeout(() => {
              if (recognitionRef.current && isRecordingRef.current) {
                try {
                  ...
                  console.log('Auto-switched recognition language to:', newLang);
                } catch (e) {
      ...
      if (finalTranscript) {
        ...
        const fullTranscript = normalizedFinal + (interimTranscript ? ' ' + interimTranscript : '');
        const normalizedFull = options.enableTextNormalization ? normalizeText(fullTranscript) : fullTranscript;
        ...
        // Update full transcript with final + interim
        const fullTranscript = finalTranscriptRef.current.trim() + ' ' + normalizedInterim;
        const normalizedFull = options.enableTextNormalization ? normalizeText(fullTranscript) : fullTranscript;
        setTranscript(normalizedFull.trim());
    ...
    recognition.onend = () => {
      ...
      const normalizedFinal = options.enableTextNormalization ? normalizeText(finalText) : finalText;
      setFinalTranscript(normalizedFinal);
  ...
  }, [options.continuous, options.interimResults, options.language, options.languages, options.autoSwitchLanguage]);
