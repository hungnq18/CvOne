...
`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional job description analyzer. Always respond with valid JSON format.'
          },
          {
            role: 'user',
            content: prompt
      ...
      this.logger.error(`Error analyzing job description: ${error.message}`, error.stack);
...
`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional CV writer. Create compelling and relevant professional summaries.'
          },
          {
            role: 'user',
            content: prompt
      ...
      this.logger.error(`Error generating professional summary: ${error.message}`, error.stack);
  ...
  async generateWorkExperience(
    ...
    try {
      const years = experienceLevel === 'senior' ? 5 : experienceLevel === 'mid-level' ? 3 : 1;
...
`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional CV writer. Create realistic and compelling work experience entries.'
          },
          {
            role: 'user',
            content: prompt
      ...
      this.logger.error(`Error generating work experience: ${error.message}`, error.stack);
      
      // Check if it's a quota exceeded error
      if (error.message.includes('429') || error.message.includes('quota')) {
        this.logger.warn('OpenAI quota exceeded, using fallback work experience');
      }
  ...
  async generateSkillsSection(
    ...
    try {
      const existingSkills = userSkills?.map(s => s.name).join(', ') || 'None';
...
`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional CV writer. Create relevant skills sections with appropriate ratings.'
          },
          {
            role: 'user',
            content: prompt
      ...
      this.logger.error(`Error generating skills section: ${error.message}`, error.stack);
    ...
    try {
      ...
      if (error.message.includes('429') || error.message.includes('quota')) {
        return {
          ...
          message: 'OpenAI quota exceeded. Please check your billing and add credits to your account.',
          error: error.message
        };
  ...
  private fallbackAnalysis(jobDescription: string) {
    ...
    return {
      ...
      experienceLevel: lowerDescription.includes('senior') ? 'senior' : 
                      lowerDescription.includes('junior') ? 'junior' : 'mid-level',
  ...
  private generateFallbackWorkExperience(jobAnalysis: any, experienceLevel: string): Array<any> {
    const years = experienceLevel === 'senior' ? 5 : experienceLevel === 'mid-level' ? 3 : 1;
    const startDate = new Date(Date.now() - years * 365 * 24 * 60 * 60 * 1000);
    const endDate = new Date();
  ...
  private generateFallbackSkills(jobAnalysis: any): Array<{name: string, rating: number}> {
    const allSkills = [...(jobAnalysis.requiredSkills || []), ...(jobAnalysis.technologies || [])];
