...
export class CvPdfService {
  ...
  async createOptimizedCvPdfWithOriginalLayout(
    ...
    try {
      ...
      doc.registerFont('Roboto-Italic', path.join(fontDir, 'Roboto-Italic.ttf'));
      ...
      this.addEnhancedSummarySection(doc, optimizedCv.userData.summary, jobAnalysis);
      ...
      this.addEnhancedWorkExperienceSection(doc, optimizedCv.userData.workHistory, jobAnalysis);
      
      // Education (optimized)
      this.addEnhancedEducationSection(doc, optimizedCv.userData.education);
      
      // Key Achievements Section (new)
      this.addKeyAchievementsSection(doc, optimizedCv.userData.workHistory, jobAnalysis);

      // Job Requirements Match Analysis (enhanced)
      this.addEnhancedJobRequirementsMatch(doc, optimizedCv.userData, jobAnalysis);

      doc.end();

      return new Promise((resolve, reject) => {
        stream.on('finish', () => {
          this.logger.log(`Enhanced optimized PDF with original layout generated successfully: ${outputPath}`);
          resolve({ success: true, filePath: outputPath });
        });
   ...
   */
  private addEnhancedSummarySection(doc: PDFKit.PDFDocument, summary: string, jobAnalysis: any) {
    doc.fontSize(16)
       .font('Roboto-Bold')
   ...
   */
  private addEnhancedSkillsSection(doc: PDFKit.PDFDocument, skills: Array<{ name: string; rating: number }>, jobAnalysis: any) {
    doc.fontSize(16)
       .font('Roboto-Bold')
    ...
    if (skills && skills.length > 0) {
      ...
      const highPrioritySkills = skills.filter(skill => 
      ...
      const otherSkills = skills.filter(skill => 
        !highPrioritySkills.some(high => high.name === skill.name)
      );
      ...
      if (highPrioritySkills.length > 0) {
        ...
        highPrioritySkills.forEach((skill, index) => {
          ...
          if (currentRow.length === skillsPerRow || index === highPrioritySkills.length - 1) {
            doc.fontSize(11)
               .font('Roboto')
               .text(currentRow.join('    '));
            currentRow = [];
      ...
      if (otherSkills.length > 0) {
        ...
        otherSkills.forEach((skill, index) => {
          ...
          if (currentRow.length === skillsPerRow || index === otherSkills.length - 1) {
            doc.fontSize(11)
               .font('Roboto')
               .text(currentRow.join(' • '));
            currentRow = [];
   ...
   */
  private addEnhancedWorkExperienceSection(doc: PDFKit.PDFDocument, workExperience: Array<any>, jobAnalysis: any) {
    doc.fontSize(16)
       .font('Roboto-Bold')
      ...
      workExperience.forEach((job, index) => {
        ...
        sentences.forEach((sentence, sentIndex) => {
          ...
          if (trimmedSentence) {
            ...
            const achievementKeywords = ['achieved', 'increased', 'improved', 'developed', 'led', 'managed', 'delivered', 'reduced', 'optimized', 'implemented'];
   ...
   */
  private addEnhancedEducationSection(doc: PDFKit.PDFDocument, education: Array<any>) {
    doc.fontSize(16)
       .font('Roboto-Bold')
       .text('Education');
   ...
   */
  private addKeyAchievementsSection(doc: PDFKit.PDFDocument, workHistory: Array<any>, jobAnalysis: any) {
    doc.fontSize(16)
       .font('Roboto-Bold')
    ...
    workHistory.forEach(job => {
      ...
      sentences.forEach(sentence => {
        ...
        if (achievementKeywords.some(keyword => trimmedSentence.toLowerCase().includes(keyword))) {
    ...
    if (achievements.length > 0) {
      ...
      doc.fontSize(12)
         .font('Roboto')
         .text('No specific achievements listed');
    }
   ...
   */
  private addEnhancedJobRequirementsMatch(doc: PDFKit.PDFDocument, cvData: any, jobAnalysis: any) {
    doc.fontSize(16)
       .font('Roboto-Bold')
       .text('Job Requirements Analysis');
